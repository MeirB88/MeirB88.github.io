...
if plat == 'android':
    ...
    #loc1=os.path.join(translatepath('special://temp'),'libtdjni.so')
    ...
    #loc4=os.path.join(translatepath('special://temp'),'libtdjson.so')

    loc4=os.path.join(translatepath('special://xbmc'),'libtdjson.so')
    ...
    if 1:#for items in all_option:
        try:
            ...
            cwd=translatepath('special://xbmc')
...
def td_receive():
    log.warning('td_json_client_receive')
    log.warning(client)
    
    result = td_json_client_receive(client, 1.0)
    log.warning('Done td_json_client_receive')
    if result:
...
'''
log.warning('Done client_create')
