...
def addDir(name, url, mode, iconimage='DefaultFolder.png', infos=None, contextMenu=None, module='', moreData='', totalItems=None, isFolder=True, isPlayable=False, addFav=True, urlParamsData={}):
	# xbmc.executebuiltin((u'Notification(%s,%s)' % ('Kodi Anonymous', quote_plus(url))))
	urlParams = {'name': name.replace('?', '|||'), 'url': quote_plus(url), 'mode': mode.replace('2','274'), 'iconimage': quote_plus(iconimage), 'module': module, 'moredata': quote_plus(moreData)}
	...
	if totalItems is None:
		xbmcplugin.addDirectoryItem(handle=handle, url=u, listitem=listitem, isFolder=False)
	else:
		xbmcplugin.addDirectoryItem(handle=handle, url=u, listitem=listitem, isFolder=False, totalItems=totalItems)
