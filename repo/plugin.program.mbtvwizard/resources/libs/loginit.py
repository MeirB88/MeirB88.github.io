    ...
    if who == 'all':
        for log in ORDER:
            if os.path.exists(LOGINID[log]['path']):
                ...
                logging.log('[מידע כניסה] {0}({1}) אינו מותקן'.format(LOGINID[log]['name'], LOGINID[log]['plugin']), level=xbmc.LOGERROR)
        ...
        if LOGINID[who]:
            ...
            logging.log('[מידע כניסה] ערך לא חוקי: {0}'.format(who), level=xbmc.LOGERROR)
    ...
    if who == 'all':
        ...
        if os.path.exists(file):
            ...
            logging.log_notify('[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, LOGINID[who]['name']),
                               '[COLOR {0}]מידע כניסה: הוסר![/COLOR]'.format(CONFIG.COLOR2),
                               2000,
                               LOGINID[who]['icon'])
    ...
    if do == 'update':
        if not user == '':
            try:
                ...
                logging.log('נתוני כניסה נשמרו עבור {0}'.format(name), level=xbmc.LOGINFO)
            except Exception as e:
                logging.log("[נתוני כניסה] לא ניתן לעדכן {0} ({1})".format(who, str(e)), level=xbmc.LOGERROR)
        else:
            logging.log('נתוני כניסה לא רשומים עבור {0}'.format(name))
    elif do == 'restore':
        if os.path.exists(file):
            ...
            try:
                ...
                user = addonid.getSetting(default)
                CONFIG.set_setting(saved, user)
                logging.log('נתוני כניסה שוחזרו עבור {0}'.format(name), level=xbmc.LOGINFO)
            except Exception as e:
                logging.log("[מידע כניסה] לא ניתן לשחזר {0} ({1})".format(who, str(e)), level=xbmc.LOGERROR)
        else:
            logging.log('נתוני כניסה לא נמצאו עבור {0}'.format(name))
    elif do == 'clearaddon':
        logging.log('{0} הגדרות: {1}'.format(name, settings), level=xbmc.LOGDEBUG)
        if os.path.exists(settings):
            try:
                ...
                logging.log("[נתוני Trakt] לא ניתן לנקות את התוסף {0} ({1})".format(who, str(e)), level=xbmc.LOGERROR)
    xbmc.executebuiltin('Container.Refresh()')
        ...
        if os.path.exists(LOGINID[who]['path']):
            ...
            if not u or u == '':
                ...
                if dialog.yesno(CONFIG.ADDONTITLE,
                                    "האם ברצונך לשמור את [COLOR {0}]מידע הכניסה[/COLOR] עבור [COLOR {1}]{2}[/COLOR]?".format(CONFIG.COLOR2, CONFIG.COLOR1, n)
...
def import_list(who):
    if who == 'all':
        ...
        if os.path.exists(LOGINID[who]['file']):
            ...
            logging.log_notify("[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, name),
                       '[COLOR {0}]נתוני כניסה: יובאו![/COLOR]'.format(CONFIG.COLOR2))
...
def activate_login(who):
    if LOGINID[who]:
        if os.path.exists(LOGINID[who]['path']):
            ...
            dialog.ok(CONFIG.ADDONTITLE, '{0} אינו מותקן כרגע.'.format(LOGINID[who]['name']))
    else:
        xbmc.executebuiltin('Container.Refresh()')
        return
