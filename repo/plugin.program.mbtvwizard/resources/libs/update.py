    ...
    if response:
        ...
        if ver > CONFIG.ADDON_VERSION:
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]קיימת גרסה חדשה של {1}!'.format(CONFIG.COLOR2, CONFIG.ADDONTITLE)
                                   +'\n'+'האם ברצונך להוריד [COLOR {0}]v{1}[/COLOR]?[/COLOR]'.format(CONFIG.COLOR1, ver),
                                   nolabel='[B][COLOR red]הזכר לי מאוחר יותר[/COLOR][/B]',
                                   yeslabel="[B][COLOR springgreen]עדכן אשף[/COLOR][/B]")
            if yes:
                from resources.libs import db
                from resources.libs.common import tools

                logging.log("[Auto Update Wizard] מתקין את גרסת האשף v{0}".format(ver))
                progress_dialog.create(CONFIG.ADDONTITLE, '[COLOR {0}]מוריד עדכון...'.format(CONFIG.COLOR2)
                                        +'\n'+''+'\n'+'נא להמתין[/COLOR]')
                ...
                xbmc.sleep(2000)
                progress_dialog.update(0, '\n'+"מתקין את עדכון {0}".format(CONFIG.ADDONTITLE))
                ...
                logging.log_notify(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]התוסף עודכן[/COLOR]'.format(CONFIG.COLOR2))
                logging.log("[Auto Update Wizard] אשף עודכן לגרסה v{0}".format(ver))
                tools.remove_file(os.path.join(CONFIG.ADDON_DATA, 'settings.xml'))
                window.show_save_data_settings()
            else:
                logging.log("[Auto Update Wizard] התקנת אשף חדש התעלמה: {0}".format(ver))
        else:
            logging.log("[Auto Update Wizard] אין גרסה חדשה v{0}".format(ver))
    else:
        logging.log("[Auto Update Wizard] כתובת URL לקובץ האשף לא חוקית: {0}".format(CONFIG.BUILDFILE))
...
def toggle_addon_updates():
    ...
    setting = '"general.addonupdates"'
    selected = 0
    options = ['התקן עדכונים אוטומטית', 'הודע לי, אך אל תתקין עדכונים', 'לעולם אל תבדוק עדכונים']
    ...
    logging.log_notify(CONFIG.ADDONTITLE, 'עדכונים שונו ל"{0}"'.format(options[selected]))
